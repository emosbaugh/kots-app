...
deps-vendor-cli:
	...
	mkdir -p deps/; \
	if [[ "`uname`" == "Linux" ]]; then curl -fsSL https://github.com/replicatedhq/replicated/releases/download/v0.20.0/replicated_0.20.0_linux_amd64.tar.gz | tar xvz -C deps; exit 0; fi; \
	if [[ "`uname`" == "Darwin" ]]; then curl -fsSL https://github.com/replicatedhq/replicated/releases/download/v0.20.0/replicated_0.20.0_darwin_amd64.tar.gz | tar xvz -C deps; exit 0; fi; fi;
